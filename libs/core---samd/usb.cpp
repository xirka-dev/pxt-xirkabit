...
static int writeNum(char *buf, uint32_t n, bool full) {
    int i = 0;
    int sh = 28;
    while (sh >= 0) {
        int d = (n >> sh) & 0xf;
...
void set_usb_strings(const char *uf2_info) {
    ...
    static char serial[(4*8+1)];
    {
        ...
        for (int i = 0; i < 4; i++) {
            serial_number_idx += writeNum(&(serial[serial_number_idx]), *(addresses[i]), true);
        }
        serial[serial_number_idx] = '\0';
    }
